...
for i in {1..5}; do
  node verify-bridge.mjs $TOPHASH1 && break || { echo "Attempt $i failed, retrying in 5 seconds..."; sleep 5; }
done
...
for i in {1..5}; do
  node verify-bridge.mjs $TOPHASH2 && break || { echo "Attempt $i failed, retrying in 5 seconds..."; sleep 5; }
done
